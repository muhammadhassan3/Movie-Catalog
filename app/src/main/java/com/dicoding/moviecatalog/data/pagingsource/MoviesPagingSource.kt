...
class MoviesPagingSource(val dataSource: RemoteDataSource): PagingSource<Int, FilmModel>() {
    ...
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, FilmModel> {
        ...
        return try {
            val response = dataSource.loadMovies(pageIndex)
                ...
                Status.SUCCESS -> {
                    if (response.data != null) {
                        val nextkey = if (response.data.isEmpty() || pageIndex==500) {
                            ...
                            pageIndex + (response.data.size / Constant.NETWORK_PAGE_SIZE)
                        }
                        LoadResult.Page(
                            data = response.data,
